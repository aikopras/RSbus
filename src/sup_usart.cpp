...
//            2026-07-27 ap V0.4 No USART precompiler error changed into a warning
//            2026-07-28 ap V0.5 Tiny0 and Tiny1 support added (14/20/24-pins only)
//
...
void USART::init(uint8_t usartNumber, bool defaultPins) {
    ...
    switch (usartNumber) {
      case 0:
          ...
          // Determine which pin should be used for TX and set this pin as output
            ...
            // tinyAVR 0/1-series: PORTMUX.CTRLB, single USART0 bit (not USARTROUTEA)
            // Default TX = PB2, alternate TX = PA1 (14/20/24-pin packages; 8-pin parts use PA6 default)
          ...
          #else
            // AVR DA/DB/DD, megaAVR-0-serie, tinyAVR 2-serie
  ...
  //****************************************************************************************************
  // Raise a warning if no USART support is found
  //****************************************************************************************************
  #ifndef HAS_USART
  #warning RS-Bus library: No USART could be found that is supported by this processor
  (void)usartNumber;               // to avoid additioal compiler warnings
  (void)defaultPins;               // to avoid additioal compiler warnings
  #endif
